#!/usr/bin/env bash
# Validation Pass 1: rerun visualize_blob_gates on ranking-sample intervals
...
OUTPUT_DIR="${REPO_ROOT}/output_smoke/blob_refine_fix/2026-05-23/validation/pass1"
...
# Initialize log
cat > "${SWEEP_LOG}" <<'EOF'
Pass 1 validation run: ranking-sample intervals
================================================
EOF
...
# Read CSV and process ranking-sample rows
{
	...
	while IFS=',' read -r rng_seed sample video interval_start interval_end bucket tier agreement blob_accept_fwd blob_accept_bwd actual_accept_fwd actual_accept_bwd rebucket_note; do
		...
		# Filter to ranking sample only
		[[ "${sample}" != "ranking" ]] && continue
...
# Final summary
{
	...
	echo "Pass 1 Summary"
	echo "============="
	echo "Total ranking-sample intervals: ${TOTAL}"
