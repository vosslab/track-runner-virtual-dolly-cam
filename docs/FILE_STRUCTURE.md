...
- Per-video state files (seeds JSON, geometry NPZ, interval-scores JSON,
  debug-tracks NPZ, camera-motion NPZ) live under the per-video `tr_config`
  store. See [docs/TR_CONFIG_FILES.md](TR_CONFIG_FILES.md).
- Encoded output videos land in the per-video output directory computed by
