...
- Adjacent Hermite intervals could estimate different tangents at the same shared seed.
...
- Commit continuously to one FWD/BWD pass over a disagreement run using the actual production
  residual evaluator.
- Compute local Hermite tangents once per shared seed and use them on both adjacent intervals.
...
- Verification strategy for uncertain methods: use inline synthetic transitions, small captured
  numeric fixtures, fake readers, deterministic residual fields, and debug output written to
...
- Replace index-selected Hermite tangent regression with a frame-distance window, share one tangent
  per seed, decide the far-endpoint chord rule, and collapse duplicated propagation code.
...
| M5 | Generated known-derivative evidence accepts the shared-tangent/chord decision | Complete |
...
- `track_runner/seed_tangents.py` owns one immutable tangent per human seed.
...
- C6: shared tangents are computed from seeds before interval dispatch.
...
| M5 / WS-TANGENT | tangent field and propagation | local derivative behavior and C6 |
...
| M5 | Tangent continuity | Use local, shared seed tangents | No derivative seam at shared seeds |
...
- Exit criteria:
  ...
  - Synthetic FWD/BWD geometry produces the same owner value in solve, analyze, encode, crop, and
    scoring seams.
...
- Depends on: M2 -- tangent changes feed agreement/promotion.
- Deliverables: frame-distance window, shared tangent field, autonomous chord decision, and one
  direction-parameterized propagator.
- Workstreams: WS-TANGENT.
- Entry criteria: M2 behavior passes.
- Exit criteria:
  - Generated irregularly spaced seeds exclude a deliberate distant outlier and recover the known
    local polynomial derivative within numerical tolerance.
  - Adjacent intervals use the same tangent object/value at their shared seed and therefore have
    zero first-derivative mismatch within numerical tolerance.
  - The chord/shared-endpoint choice is made by the lower derivative error on generated curves with
    known derivatives; a tie selects the simpler shared-tangent rule.
  - FWD/BWD propagation tests cover both directions through the unified implementation.
- Parallel-plan ready: no -- the window, shared field, and chord decision change the same geometry
  in sequence.
...
- Depends on: M2 and M5 -- confidence and trajectory derivatives are inputs.
...
- Exit criteria:
  ...
  - The retained repository-owned numeric trajectory fixture runs `direct_center`, `smooth`, and
    `dolly` from identical in-memory inputs.
  - Dolly is adopted automatically when it converges without fallback, preserves containment, has
    no greater lag than both baselines, and has lower p95 acceleration than both. If the rule fails,
    `direct_center` remains the default and the rejection reason is recorded; no user reply is
    required either way.
...
### Workstream: WS-TANGENT tangent continuity

- Goal: finish M5 on generated curves with known derivatives.
- Owner: `expert_coder`.
- Work packages: WP-N1 through WP-N4.
- Interfaces:
  - Needs: seeds and fps.
  - Provides: immutable shared tangents to interval workers.
- Review boundary: `architect` checks C6 and derivative semantics.
...
- Interfaces:
  - Needs: synthetic trajectories and the retained numeric fixture.
...
| WP-N1 | expert_coder | WP-T3 | local window recovers known derivatives and rejects distant support |
| WP-N2 | expert_coder | WP-N1 | adjacent intervals share one tangent per seed |
| WP-N3 | expert_coder | WP-N2 | generated known-derivative cases select the chord/shared rule automatically |
...
| WP-D3 | tester | WP-D2 | retained numeric fixture applies the automatic default rule |
...
- Global suite policy: the completed portable suite passes 4,559 tests. The
  user-promoted source-file ownership maintenance work cleared the exclusive
...
- Generated polynomial trajectories for tangent and dolly math.
...
- The small retained numeric crop trajectory fixture for the three-mode adoption rule.
...
| Dolly rule overfits one trajectory | Poor default | Synthetic regime fails | tester | Require retained fixture plus stationary, velocity, acceleration, gap, and containment cases |
...
- Patch 3: M5 tangent validation and autonomous chord-rule confirmation.
...
algorithms remain unchanged. Review corrected canonical approximate/legacy-
obstructed seed eligibility with a real NPZ identity round-trip and made
