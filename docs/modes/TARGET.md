...
```text
usage: track_runner.py target [-h] [--race-start | --from-analyze]
                              [-s {high,medium,low}] [-t TOP_N] [-g GAP_TOP_N]
                              [-I SEED_INTERVAL] [-S START_TIME]

options:
  -h, --help            show this help message and exit
  --race-start          Target frames around the detected race-start
                        transition for confirmation.
  --from-analyze        Target frames from the latest 'analyze' report: union
...
**Mutual exclusivity:** `--race-start` and `--from-analyze` are mutually exclusive; use only one per invocation.
