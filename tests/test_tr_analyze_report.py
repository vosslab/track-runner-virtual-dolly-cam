...
#============================================
def _call_write(monkeypatch, tmp_path, **overrides):
...
def test_writes_html_at_supplied_out_path(tmp_path, monkeypatch):
...
def test_html_contains_required_structural_markers(tmp_path, monkeypatch):
...
def test_canvas_ids_appear_in_spec_order(tmp_path, monkeypatch):
...
def test_renderer_script_follows_json_data_block(tmp_path, monkeypatch):
...
def test_html_contains_video_stem_in_h1(tmp_path, monkeypatch):
...
def test_embedded_json_is_valid_json(tmp_path, monkeypatch):
...
def test_html_contains_no_external_url(tmp_path, monkeypatch):
...
def test_html_is_ascii_only(tmp_path, monkeypatch):
...
def test_warnings_section_omitted_when_warnings_empty(tmp_path, monkeypatch):
...
def test_warnings_section_present_when_warnings_supplied(tmp_path, monkeypatch):
...
def test_warnings_html_escaped(tmp_path, monkeypatch):
...
def test_running_mean_nan_aware_constant_input_passes_through():
...
def test_running_mean_nan_aware_single_nan_does_not_blank_window():
...
def test_running_mean_nan_aware_all_nan_window_yields_nan():
...
def test_runner_speed_per_frame_uses_cx_cy_when_present():
...
def test_runner_speed_per_frame_constant_velocity():
...
def test_runner_speed_per_frame_handles_missing_frame_data():
...
def test_runner_speed_panel_data_shape():
...
def test_runner_speed_panel_data_no_fps_omits_secondary_axis():
...
def test_zoom_stability_smoothed_passes_through_when_input_smooth():
...
def test_zoom_stability_smoothed_attenuates_alternating_input():
...
def test_zoom_stability_panel_shape():
...
def test_zoom_multiple_constant_input_flat_achieved_multiple():
...
def test_zoom_multiple_omits_reference_lines_when_config_missing_key():
...
def test_zoom_multiple_emits_reference_line_when_config_provides_value():
...
def test_camera_motion_constant_dxy_flat_magnitude():
...
def test_camera_motion_panel_shape():
...
def test_full_html_when_all_inputs_present(tmp_path, monkeypatch):
...
def test_html_when_motion_missing(tmp_path, monkeypatch):
...
def test_html_when_scene_transform_missing(tmp_path, monkeypatch):
...
def test_html_no_external_url_under_all_degradation_cases(tmp_path, monkeypatch):
...
def test_embedded_json_series_lengths_match_frames(tmp_path, monkeypatch):
...
def test_html_when_trajectory_empty(tmp_path, monkeypatch):
...
def test_zoom_panels_require_aligned_trajectory_and_crop_rects():
...
def test_integration_full_html_under_full_inputs(tmp_path, monkeypatch):
...
def test_integration_motion_missing_skips_camera_and_speed(tmp_path, monkeypatch):
...
def test_integration_trajectory_padded_to_match_crop_rects(tmp_path, monkeypatch):
...
def test_integration_scene_transform_missing_skips_speed(tmp_path, monkeypatch):
