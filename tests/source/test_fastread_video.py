...
def test_fastread_path_ends_with_fastread_mkv(tmp_path: object) -> None:
	...
	original = str(tmp_path) + "/runner_source.mkv"
	result = tr_paths.fastread_video_path(original)
	# must end with the expected suffix
	assert result.endswith("runner_source.fastread.mkv")
...
def test_config_path_uses_original_stem() -> None:
	...
	assert "runner_source" in os.path.basename(config)
	assert "fastread" not in os.path.basename(config)

def test_torso_box_coords_path_uses_original_stem() -> None:
	"""Torso-coordinate artifact stem matches original basename, not fastread."""
	original = "/data/runner.mkv"
	intervals = tr_paths.default_torso_box_coords_path(original)
	assert "runner" in os.path.basename(intervals)
...
def test_contact_sheet_path_keyed_off_original_not_decode() -> None:
	...
	assert "runner_source" in os.path.basename(artifact)
...
def _make_original(tmp_path: object) -> str:
	...
	original_path = os.path.join(str(tmp_path), "runner_source.mkv")
...
def test_analyze_yaml_carries_source_fields(tmp_path: object) -> None:
	...
	canonical_source = "runner_source.mkv"
	decode_source = "runner_source.fastread.mkv"
...
def test_analyze_html_carries_source_fields(tmp_path: object) -> None:
	...
	assert "runner_source.fastread.mkv" in html_text
	assert "runner_source.mkv" in html_text
