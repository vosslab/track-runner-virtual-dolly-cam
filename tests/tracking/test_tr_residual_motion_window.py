...
def test_resolve_stride_60fps_anchor() -> None:
	"""60 fps anchor: stride=1 covers consecutive frames."""
	result = residual_motion.resolve_stride(60.0)
	assert result == 1, f"Expected 1, got {result}"


def test_resolve_stride_fps_zero_raises() -> None:
	"""fps <= 0 raises ValueError."""
	with pytest.raises(ValueError):
		residual_motion.resolve_stride(0.0)


def test_resolve_stride_fps_negative_raises() -> None:
	"""fps < 0 raises ValueError."""
	with pytest.raises(ValueError):
		residual_motion.resolve_stride(-1.0)


def test_resolve_stride_fps_none_raises() -> None:
	"""fps=None raises ValueError."""
	with pytest.raises(ValueError):
		residual_motion.resolve_stride(None)


def test_resolve_stride_returns_int() -> None:
...
def test_resolve_stride_monotone() -> None:
...
def test_neighbor_count_fixed_regardless_of_fps() -> None:
