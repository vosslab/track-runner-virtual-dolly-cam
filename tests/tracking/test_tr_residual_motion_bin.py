...
class _StubReader:
	...
	def __init__(self, geometry: common_tools.frame_reader.FrameGeometry) -> None:
	...
	def read_frame(self, idx: int) -> object:  # pragma: no cover - never called via stubs
		return numpy.zeros((self.height, self.width, 3), dtype=numpy.uint8)


#============================================
def _patch_inner_pipeline(
	monkeypatch: pytest.MonkeyPatch,
	blob_proc_xy: tuple[float, float],
	blob_score: float = 0.7,
) -> None:
	...
	def _stub_residual(
		...
		half_window: int, frame_read_cache: object, roi: object,
		fps: object = None, stride: object = None,
	) -> object:
		h = reader.height
		w = reader.width
	...
	def _stub_dog(residual: object, pred_w: float, k: float) -> object:
		return residual
	...
	# yields (bx, by).
	bx, by = blob_proc_xy
	def _stub_extract(dog: object, validity: object, threshold: float) -> object:
		# We accept any ROI; observe_blob_at will set roi[0],roi[1]
		# from _compute_roi. For the test we use a static synthetic
		# blob and let the observation path select it.
		return [{
	...
	def _stub_confidence(
		blob: object, cx: float, cy: float, w: float, h: float,
	) -> float:
...
#============================================
def _observe_at_geometry(
	monkeypatch: pytest.MonkeyPatch,
	geometry: common_tools.frame_reader.FrameGeometry,
	proc_pred_xy: tuple[float, float],
	proc_box_wh: tuple[float, float],
	blob_proc_xy: tuple[float, float],
) -> object:
	...
	# (already done above; the extraction stub delivers raw blobs untouched and
	# observe_blob_at adds the ROI offset.
	...
	def _stub_extract(
		dog: object, validity: object, threshold: float,
		_bx: float = bx, _by: float = by,
	) -> object:
		return [{
...
#============================================
def test_observe_blob_at_returns_source_frame_at_bin1(
	monkeypatch: pytest.MonkeyPatch,
) -> None:
...
#============================================
def test_observe_blob_at_returns_source_frame_at_bin2(
	monkeypatch: pytest.MonkeyPatch,
) -> None:
...
#============================================
def test_observe_blob_at_cache_key_is_processed_frame(
	monkeypatch: pytest.MonkeyPatch,
) -> None:
