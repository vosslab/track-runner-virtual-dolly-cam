...
#============================================
def _bundle(
	direction_sign: int, start_frame: int, end_frame: int, seed: dict, neighbor: dict,
) -> walker_bundle.WalkerInputBundle:
...
#============================================
def _fake_summary(direction_path: list[dict]) -> object:
	"""Stand-in for WalkSummary: the adapter only reads .direction_path."""
	return types.SimpleNamespace(direction_path=direction_path)


#============================================
def _patch_walker(monkeypatch: pytest.MonkeyPatch, direction_path: list[dict]) -> None:
...
#============================================
def test_complete_walk_produces_full_span_anchored_path(
...
#============================================
def test_short_walk_is_padded_to_full_interval_span(monkeypatch: pytest.MonkeyPatch) -> None:
...
#============================================
def test_both_bundle_adapters_forward_exact_precomputed_store(
	monkeypatch: pytest.MonkeyPatch,
) -> None:
	"""Both production adapters forward the bundle's exact cache sentinel."""
	seed_start = _seed(0, 0.0, 0.0, 20.0, 40.0)
	seed_end = _seed(2, 20.0, 0.0, 20.0, 40.0)
	sentinel = object()
	bundle = _bundle(1, 0, 2, seed_start, seed_end)
	bundle.precomputed_store = sentinel
	direction_path = [
		{"frame_index": 0, "cx": 0.0, "cy": 0.0, "w": 20.0, "h": 40.0,
			"conf": 1.0, "status": "accepted"},
	]
	...
	def fake_walk(**kwargs: object) -> object:
		received.append(kwargs["precomputed_store"])
