...
#============================================
def test_parse_resolution_WxH_round_trip() -> None:
	# parse WxH into [W, H]; this is the one real parsing step
	assert encode_mode._parse_resolution("1920x1080") == [1920, 1080]
	assert encode_mode._parse_resolution("1280X720") == [1280, 720]


#============================================
def test_parse_resolution_rejects_bad_forms() -> None:
	# error detection: missing 'x', wrong separator, empty sides
	with pytest.raises(RuntimeError, match="WxH"):
		encode_mode._parse_resolution("1920")
	with pytest.raises(RuntimeError, match="WxH"):
		encode_mode._parse_resolution("1920,1080")
	with pytest.raises(RuntimeError, match="WxH"):
		encode_mode._parse_resolution("x1080")


#============================================
# Default output path: extension is forced to lowercase '.mkv'.
def test_default_output_path_forces_mkv_extension() -> None:
...
def test_resolve_filters_no_filters_alone() -> None:
	args = _make_args(no_filters=True)
	cfg = {"encode_filters": ["bilateral"]}
	assert encode_mode._resolve_encode_filters(args, cfg) == []


def test_resolve_filters_no_filters_conflicts_with_dash_F() -> None:
	args = _make_args(no_filters=True, encode_filters="bilateral")
	with pytest.raises(RuntimeError, match="cannot be combined"):
		encode_mode._resolve_encode_filters(args, {})


def test_resolve_filters_dash_F_none_alias() -> None:
	args = _make_args(encode_filters="none")
	assert encode_mode._resolve_encode_filters(args, {"encode_filters": ["bilateral"]}) == []


def test_resolve_filters_dash_F_none_case_insensitive() -> None:
	args = _make_args(encode_filters="  NONE  ")
	assert encode_mode._resolve_encode_filters(args, {}) == []


def test_resolve_filters_rejects_mixed_none() -> None:
...
def test_encode_parser_accepts_draw_flags() -> None:
...
def test_encode_parser_accepts_debug_overlay() -> None:
...
def test_encode_parser_rejects_both_overlay_tiers() -> None:
	with pytest.raises(SystemExit):
		_parse(["-i", "x.MOV", "encode",
			"--draw-tracking-overlay", "--draw-debug-overlay"])


def test_encode_parser_rejects_local_d() -> None:
	# global -d must come before the subcommand; encode -d should fail
	with pytest.raises(SystemExit):
		_parse(["-i", "x.MOV", "encode", "-d"])


def test_global_debug_does_not_set_overlay_flags() -> None:
...
def test_velocity_arrow_alone_rejected() -> None:
	with pytest.raises(SystemExit):
		_parse(["-i", "x.MOV", "encode", "--draw-velocity-arrow"])


def test_mp4_with_explicit_mkv_output_rejected() -> None:
	with pytest.raises(SystemExit):
		_parse(["-i", "x.MOV", "encode", "--mp4", "-o", "out.mkv"])


def test_explicit_mp4_output_accepted() -> None:
	args = _parse(["-i", "x.MOV", "encode", "-o", "out.mp4"])
	assert args.output_file == "out.mp4"


def test_uppercase_output_extension_accepted() -> None:
...
#============================================
def test_debug_overlay_reports_m3_commitment_direction_and_alpha(
	monkeypatch: pytest.MonkeyPatch,
) -> None:
	"""Developer overlays make M3's committed-pass decision reviewable."""
	labels = []
	original_put_text = encoder.cv2.putText

	def record_put_text(*args, **kwargs) -> object:
		labels.append(args[1])
		return original_put_text(*args, **kwargs)

	monkeypatch.setattr(encoder.cv2, "putText", record_put_text)
	...
	state = {
		...
		"h": 40.0,
		"conf": 0.5,
		"source": "committed_fwd",
	}
	encoder.draw_debug_overlay_cropped(
		frame, state, (0, 0, 160, 120), 160, 120,
		debug_state={"commitment_direction": "fwd", "commitment_alpha": 0.5},
		draw_tracking=True, draw_debug=True,
	)
	assert "commit:fwd 50%" in labels


#============================================
def test_debug_projection_stitch_preserves_pre_race_absence() -> None:
	"""A pre-race interval has blended geometry but no FWD/BWD diagnostics."""
	pre_race = {
		"start_frame": 0,
		"forward_path": None,
		"backward_path": None,
		"blended_path": [_state(40.0, 30.0)],
	}
	post_race = {
		"start_frame": 1,
		"forward_path": [_state(50.0, 30.0)],
		"backward_path": [_state(60.0, 30.0)],
		"blended_path": [_state(55.0, 30.0)],
	}
	fwd, bwd = encode_mode._stitch_debug_projection_paths(
		[pre_race, post_race], 2,
	)
	assert fwd == [None, post_race["forward_path"][0]]
	assert bwd == [None, post_race["backward_path"][0]]


#============================================
def test_debug_projection_stitch_rejects_one_absent_direction() -> None:
	"""Only a matched absent pair has the pre-race meaning."""
	interval = {
		"start_frame": 0,
		"forward_path": None,
		"backward_path": [_state(60.0, 30.0)],
	}
	with pytest.raises(RuntimeError, match="only one FWD/BWD"):
		encode_mode._stitch_debug_projection_paths([interval], 1)
...
def _validate(trajectory: list, crop_rects: list, **kwargs) -> None:
...
def test_validate_torso_passes_when_centered() -> None:
...
def test_validate_torso_passes_with_short_glitch() -> None:
...
def test_validate_torso_fails_when_off_left_sustained() -> None:
...
def test_validate_torso_fails_when_off_right_sustained() -> None:
...
def test_validate_edge_attribute_left() -> None:
...
def test_validate_edge_attribute_right() -> None:
...
def test_validate_edge_attribute_empty_when_crop_fits() -> None:
...
def test_validate_torso_inclusive_bounds() -> None:
...
def test_validate_torso_skips_none_and_non_finite() -> None:
...
def test_validate_skipped_frames_pause_run() -> None:
...
def test_validate_reports_first_violation() -> None:
...
def test_validate_ignores_later_violations_after_first() -> None:
...
def test_encode_parser_accepts_allow_offcenter_crop() -> None:
	args = _parse(["-i", "x.MOV", "encode", "--allow-offcenter-crop"])
	assert args.allow_offcenter_crop is True


def test_encode_parser_default_disallows_offcenter_crop() -> None:
	args = _parse(["-i", "x.MOV", "encode"])
...
def test_max_centered_fit_passes_through_when_fits() -> None:
...
def test_max_centered_fit_horizontal_bound() -> None:
...
def test_max_centered_fit_vertical_bound() -> None:
...
def test_max_centered_fit_corner_bound_takes_smaller() -> None:
...
def test_max_centered_fit_does_not_exceed_desired() -> None:
...
def test_direct_center_keeps_runner_centered_when_zoom_too_aggressive() -> None:
...
def test_direct_center_aspect_preserved_under_fit() -> None:
...
def test_direct_center_allows_black_fill_when_flag_disabled() -> None:
	# When crop_centered_fit_to_source is false, the crop may slide off the
	# frame edge and encode fills the uncovered region with black.
	fw, fh = 2816, 1584
	...
	rects = tr_crop.direct_center_crop_trajectory(traj, fw, fh, cfg)
	# At least one frame has crop_x < 0, requiring black fill.
