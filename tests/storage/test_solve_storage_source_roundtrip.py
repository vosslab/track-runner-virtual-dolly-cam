...
def _seed(frame_index: int) -> dict:
	...
	return {
		...
		"cx": box["cx"], "cy": box["cy"],
		"w": box["w"], "h": box["h"],
		"status": "visible" if frame_index == START_FRAME else "partial",
	}
...
#============================================
def _stub_solve_collaborators(monkeypatch: pytest.MonkeyPatch) -> None:
...
def _run_walker_interval(
	monkeypatch: pytest.MonkeyPatch, bin_factor: int,
	walker_path_factory: collections.abc.Callable[[int], list], captured_seeds: list,
) -> dict:
	...
	def fake_coverage(bundle: object) -> tuple[list, walker_bundle.WalkCoverage]:
...
def _run_hermite_interval(monkeypatch: pytest.MonkeyPatch, bin_factor: int) -> dict:
	...
	def source_truth_path(*a, **k) -> list:
		path = []
		for frame_index in range(START_FRAME, END_FRAME + 1):
			box = _source_truth_box(frame_index)
			path.append({
...
#============================================
def _store_and_load_blended(tmp_path: pathlib.Path, result: dict) -> list:
	...
	torso_box_coords_io.write_torso_box_coords(npz_path, cache_data)
	loaded = torso_box_coords_io.load_torso_box_coords(npz_path)
...
#============================================
def test_walker_interval_stores_source_at_bin3(
...
#============================================
def test_walker_interval_stores_source_at_bin1(
	tmp_path: pathlib.Path, monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_human_seed_endpoints_survive_hermite_and_walker_selection(
	...
	for result in (hermite, walker):
		...
		end_geometry = tuple(
			result["blended_path"][-1][key] for key in ("cx", "cy", "w", "h")
		)
		expected_start = tuple(
			_seed(START_FRAME)[key] for key in ("cx", "cy", "w", "h")
		)
		expected_end = tuple(
			_seed(END_FRAME)[key] for key in ("cx", "cy", "w", "h")
		)
		assert start_geometry == expected_start
		assert end_geometry == expected_end
		assert result["blended_path"][0]["seed_status"] == "visible"
		assert result["blended_path"][-1]["seed_status"] == "partial"
...
#============================================
def test_scale_injection_fails_at_bin3(
	tmp_path: pathlib.Path, monkeypatch: pytest.MonkeyPatch,
) -> None:
...
#============================================
def test_swapped_axis_fails_at_bin3(
	tmp_path: pathlib.Path, monkeypatch: pytest.MonkeyPatch,
) -> None:
...
#============================================
def test_subpixel_drift_passes_at_bin3(
	tmp_path: pathlib.Path, monkeypatch: pytest.MonkeyPatch,
) -> None:
